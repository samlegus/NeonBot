...
Write-Host "[Windows 2/5] Checking nai_job help"
& $PythonBin (Join-Path $RootDir "src\nai_job.py") --help | Out-Null

Write-Host "[Windows 3/5] Checking nai_cli help"
& $PythonBin (Join-Path $RootDir "src\nai_cli.py") --help | Out-Null

Write-Host "[Windows 4/5] Checking JSON bridge dry-run"
& $PythonBin (Join-Path $RootDir "src\nai_job.py") --json '{"prompt":"smoke test","mode":"text"}' --dry-run | Out-Null

Write-Host "[Windows 5/5] Checking payload preview mode"
& $PythonBin (Join-Path $RootDir "src\nai_cli.py") --prompt "smoke test" --debug-payload | Out-Null
