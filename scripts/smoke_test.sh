...
echo "[2/5] Checking run_nai_job help"
"${PYTHON_BIN}" "${ROOT_DIR}/src/run_nai_job.py" --help >/dev/null

echo "[3/5] Checking nai_cli_v2 help"
"${PYTHON_BIN}" "${ROOT_DIR}/src/nai_cli_v2.py" --help >/dev/null

echo "[4/5] Checking JSON bridge dry-run"
"${PYTHON_BIN}" "${ROOT_DIR}/src/run_nai_job.py" \
  --json '{"prompt":"smoke test","mode":"text"}' \
  --dry-run >/dev/null

echo "[5/5] Checking payload preview mode"
"${PYTHON_BIN}" "${ROOT_DIR}/src/nai_cli_v2.py" \
  --prompt "smoke test" \
